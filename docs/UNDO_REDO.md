...
- RemoveFeatureCommand: retains the removed object and restores the same ID/order.
  Delete Feature rejects sources that still have dependents; delete dependents first.
...
- Individual deletion targets Body features; legacy Document primitives can be
  undone after creation or removed/restored together by Clear Project.
...
5. Delete the leaf feature, Undo and Redo; verify ID/order and tree selection behavior.
